...
public class MainActivity extends Activity
{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 获取屏幕高度、宽度
        getWindowManager().getDefaultDisplay().getMetrics(metric);//获得数据放metric中
        windowHeight = metric.heightPixels;  // 屏幕高度
        windowWidth = metric.widthPixels;  // 屏幕宽度
        getWindow().setSoftInputMode(
                WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);//设置键盘不会自动弹出
        res = getResources();
        // 加载main.xml界面设计文件
        ...
        mainView = new GameView(this.getApplicationContext()
                , GameView.STAGE_INIT);//创建GameView
